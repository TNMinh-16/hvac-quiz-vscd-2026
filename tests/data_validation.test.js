...
describe("questions.json validation", () => {
  ...
  test("đúng 379 câu hỏi", () => {
    expect(db.questions).toHaveLength(379);
  });
  ...
  test("tất cả 1.516 phương án có đầy đủ tiếng Anh và tiếng Việt", () => {
    const errors = [];
    let totalOptions = 0;
    for (const q of db.questions) {
    ...
    expect(totalOptions).toBe(1516);
    expect(errors).toHaveLength(0);
  });

  test("đủ 379 lời giải thích bằng cả tiếng Anh và tiếng Việt", () => {
    const errors = [];
    for (const q of db.questions) {
  ...
  test("thứ tự câu hỏi liên tục (order = 1..N)", () => {
    ...
    const expected = Array.from({ length: 379 }, (_, i) => i + 1);
    expect(orders).toEqual(expected);
  });

  test("đúng 50 liên kết ảnh-câu và ảnh bìa không bị gắn vào câu hỏi", () => {
    let totalRefs = 0;
    const errors = [];
    for (const q of db.questions) {
