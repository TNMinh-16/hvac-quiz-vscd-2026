...
// ─── Tests ────────────────────────────────────────────────────────────────────
describe("Server API", () => {
  test("GET /api/health – trả về healthy và đúng 3000 câu hỏi", async () => {
    const res = await httpGet(`${BASE}/api/health`);
    expect(res.status).toBe(200);
    expect(res.body.status).toBe("healthy");
    expect(res.body.questionCount).toBe(3000);
  });

  test("GET /api/metadata – 3000 câu, có standards", async () => {
  ...
  test("POST /api/sessions – sequential 3000 câu theo thứ tự", async () => {
